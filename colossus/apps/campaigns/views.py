...
class CampaignListView(CampaignMixin, ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        UserCampaignList = Campaign.objects.filter(added_by=self.request.user)
        #kwargs['total_count'] = Campaign.objects.count() #old
        kwargs['total_count'] = UserCampaignList.count()
    ...
    def get_queryset(self):
        ...
        queryset = super().get_queryset().filter(added_by=self.request.user)
        ...
        try:
            ...
            if status_filter in CampaignStatus.FILTERS:
                ...
                queryset = queryset.filter(added_by=self.request.user.username)
        ...
        if self.request.GET.get('q', ''):
            ...
            queryset = queryset.filter(added_by__icontains = self.request.user.username)
            self.extra_context['is_filtered'] = True
            self.extra_context['query'] = query
            #self.extra_contect['added_by'] = self.request.user.username
...
class CampaignEditView(CampaignMixin, DetailView):
    ...
    def get_queryset(self):
        return super().get_queryset() \
            ...
            .filter(added_by=self.request.user) \
...
class CampaignEditRecipientsView(CampaignMixin, UpdateView):
    ...
    def get_form_kwargs(self):
        kwargs = super(CampaignEditRecipientsView, self).get_form_kwargs()
...
def load_list_tags(request):
    ...
    try:
        user_filter = MailingList.objects.filter(added_by=request.user)
        mailing_list = user_filter.get(pk=list_id)
