...
class MailingListListView(ListView):
    ...
    def get_context_data(self, **kwargs):
        UserList = MailingList.objects.filter(added_by=self.request.user)
        kwargs['menu'] = 'lists'
        #kwargs['total_count'] = MailingList.objects.count() ##old
        kwargs['total_count'] = UserList.count()
        print(kwargs)
...
class MailingListCreateView(CreateView):
    ...
    def form_valid(self, form):##added
        ...
        return super(MailingListCreateView, self).form_valid(form)##end added
