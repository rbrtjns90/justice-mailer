...
class EmailTemplateListView(EmailTemplateMixin, ListView):
    ...
    def get_context_data(self, **kwargs):
        ##added
        added_templates = self.model.objects.filter(added_by=self.request.user)
        shared_templates = self.model.objects.filter(shared=True)
        combined_templates = added_templates | shared_templates #merge both querysets
        #kwargs['total_count'] = self.model.objects.count() #original
        kwargs['total_count'] = combined_templates.count()
        return super().get_context_data(**kwargs)
    ...
    def get_queryset(self):
        ...
        added_templates = self.model.objects.filter(added_by=self.request.user)
        shared_templates = self.model.objects.filter(shared=True)
        combined_templates = added_templates | shared_templates
        ###added
        #queryset = self.model.objects.select_related('last_used_campaign')##old
        queryset = combined_templates
...
class EmailTemplateCreateView(EmailTemplateMixin, CreateView):
    ...
    def form_valid(self, form):##added
        obj = form.save(commit=False)
        obj.added_by = self.request.user
        if self.request.POST['shared'] == 'on':
            obj.shared = True
            print(obj.shared)
        else:
            obj.shared = False
        return super(EmailTemplateCreateView, self).form_valid(form)##end added
