...
from colossus.apps.accounts.forms import RegularUserCreationForm # added
# end added
...
def user_signup(request):
    if request.method == 'POST':
        form = RegularUserCreationForm(data=request.POST)
        if form.is_valid():
            user = form.save()
            login(request, user)
    ...
    return render(request, 'core/added/signup.html', {'form': form})
